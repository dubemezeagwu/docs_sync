...
class DeltaToPDF {
  static List<Widget> _toPDFWidgets(Delta delta) {
    ...
    for (Operation deltaOp in delta.toList()) {
      if (deltaOp.isInsert) {
        ...
        if (kDebugMode) {
          print("Unsupported operation: ${deltaOp.key}");
        }
      }
    }
    if (kDebugMode) {
  ...
  static PdfColor _from8DigitHexColor(String color) {
    if (color.startsWith('#')) {
  ...
  static Widget toPDFWidget(Delta delta) {
